...
const Register = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={styles.form}>
      <h1>Регистрация</h1>
      <label
        className={nicknameError.error ? `${styles.label} ${styles.errortext}` : styles.label}
        htmlFor='nickname'
      >
        Ник:
      </label>
      <input
        className={nicknameError.error ? `${styles.input} ${styles.error}` : styles.input}
        type='text'
        {...register('nickname', {
      ...
      />
      <p className={styles.errortext}>{nicknameError.error && String(nicknameError.message)}</p>
      ...
      <label
        className={passrepeatError.error ? `${styles.label} ${styles.errortext}` : styles.label}
        htmlFor='passrepeat'
      >
        Повтор пароля:
      </label>
      <input
        className={passrepeatError.error ? `${styles.input} ${styles.error}` : styles.input}
        type='text'
        {...register('passrepeat', {
          required: 'Это поле не может быть пустым',
          minLength: 3,
      ...
      />
      <p className={styles.errortext}>{passrepeatError.error && String(passrepeatError.message)}</p>
